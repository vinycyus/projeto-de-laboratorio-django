...
def solicitar_exames(request):
    ...
    #receber dados no baçkend
    elif request.method ==  "POST":
        exames_id = request.POST.getlist('exames')
        solicitacao_exames = TiposExames.objects.filter(id__in=exames_id)

        #TODO: calcular preço do dados disponiveis
        preco_total = 0
...
def permitir_abrir_exame(request, exame_id):
    ...
    if exame.usuario == request.user:
        if not exame.requer_senha:
            # TODO: verificar se tem o pdf no resultado
            return redirect(exame.resultado.url)
        else:
            return redirect(reverse('solicitar_senha_exame', args=[exame_id]))
    else:
        messages.add_message(request, constants.ERROR, 'Esse pedido não é seu, portanto você não pode abrir')
